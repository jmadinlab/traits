    ...
    def save_unique_observations
      ...
      (2..spreadsheet.last_row).map do |i|
        ...
        if not $observation_id_map.keys().include? row["observation_id"] and row["observation_id"].present?
          ...
          observation_row = { "user_id" => row["user_id"], "private" => acc, "location_id" => row["location_id"], "specie_id" => specie_id, "resource_id" => row["resource_id"], "secondary_id" => row["secondary_id"], "approval_status" => "pending"}
